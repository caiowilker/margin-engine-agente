...
/**
 * Leiaute oficial DANFE A4 (MOC): logo na caixa do emitente, à esquerda da
 * razão social, sem expandir nem empilhar. ExpandeLogoMarca=1 + Largura/Altura=0
 * usa o tamanho em pixels da imagem e estoura o cabeçalho; LogoemCima=1 empurra
 * os dados obrigatórios para baixo.
 *
 * @param {string} logoPath
 * @returns {Array<[string, string, string]>}
 */
function logoDanfeAcbrSets(logoPath) {
  return [
...
function applyDanfeLogoAcbrLib(inst, runtime, opts = {}) {
  try {
    ...
    configGravarSafe(inst, logoDanfeAcbrSets(logoPath));
    return true;
  } catch (_) {
    return false;
  }
...
/**
 * Comandos Monitor equivalentes a {@link applyDanfeLogoAcbrLib}.
...
function danfeLogoMonitorComandos(logoPathAbs) {
  ...
  return logoDanfeAcbrSets(q).map(
