...
function atualizarStatusMemoria(ok, opts = {}) {
  ...
  // Sticky só enquanto soft-dead ativo (handle abandonado recente) — não para sempre.
  if (!ok && !degradado) {
    try {
      const session = require("./fiscal/drivers/acbrLibSession");
      if (session.isSoftDead()) {
        degradado = true;
      }
    } catch (_) {}
  }
...
async function gerarPdfFiscal(chave, xmlPath, modeloDocumento = "65", opts = {}) {
  ...
  if (deveAplicarLogoDanfe(modelo, formatoPdf) && logoInfo.ativo && logoInfo.caminhoAbsoluto) {
    ...
    logoCmds.push(...danfeLogoMonitorComandos(logoInfo.caminhoAbsoluto));
    logoCmds.push("NFE.ConfigGravar()");
  }
  const comandos = [
    ...layoutDanfe,
    ...logoCmds,
...
async function consultarChaveEntradaCore(chaveNorm, cnpjDestinatario, ufAutor, deps = null) {
  ...
  try {
    ...
    consulta = { cStat: null, xMotivo: err.message, situacao: "ERRO_CONSULTA", raw: null };
  }
